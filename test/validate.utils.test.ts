...
describe("schema validation for utils", () => {
  ...
  it("requires a non-null utils url when there's no pattern lib url", () => {
    ...
    expect(() =>
      fromObject({
        patternLibUrl: null,
        utils: { sources: [{ type: "rule", selector: ".foo" }] },
      })
    ).toThrow(jasmine.objectContaining({ name: "InvalidConfigError" }));
    expect(() =>
      fromObject({
        ...
        utils: {
          utilsUrl: null,
          sources: [{ type: "rule", selector: ".foo" }],
        },
      })
    ).toThrow(jasmine.objectContaining({ name: "InvalidConfigError" }));
    expect(() =>
    ...
    expect(() =>
      fromObject({
        ...
        utils: { utilsUrl: null, sources: [{ type: "rule", selector: "a" }] },
      })
    ).not.toThrow();
  });
  ...
  it("requires type and selector fields on utils", () => {
    expect(() =>
      fromObject({
        utils: { utilsUrl: "http://example.com", sources: [{} as any] },
      })
    ).toThrow(jasmine.objectContaining({ name: "InvalidConfigError" }));
    expect(() =>
      fromObject({
    ...
    expect(() =>
      fromObject({
        ...
        utils: {
          utilsUrl: "http://example.com",
          sources: [
            { name: "hi!", type: "rule", selector: ".foo", components: [] },
          ],
        },
      })
    ).not.toThrow();
  ...
  it("accepts only 'rule' and 'element' as types on utils", () => {
    expect(() =>
      fromObject({
        utils: {
          utilsUrl: "http://foo.com",
          sources: [{ type: "a", selector: ".foo" } as any],
        },
      })
    ).toThrow();
    expect(() =>
    ...
    expect(() =>
      fromObject({
        utils: {
          utilsUrl: "http://foo.com",
          sources: [{ type: "element", selector: ".foo" }],
        },
      })
    ).not.toThrow();
  });
